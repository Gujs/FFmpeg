...
static int     g_pll_acquire_us = 100000;    /* |ema| above this = "large" → ACQUIRE one-shot; else TRACK */
        ...
        if (filt->pts != AV_NOPTS_VALUE) {
            ...
            if (a->multiview && g_audio_follow && g_avsync_pll) {
                ...
                if (a->av_off_valid) {
                    ...
                    int64_t now = av_gettime_relative();
                    int64_t ep  = a->disturb_epoch ? atomic_load_explicit(a->disturb_epoch, memory_order_relaxed) : 0;
                    if (!a->pll_seed) { a->pll_ema = off; a->pll_dbnc_ref = off; a->pll_t0_us = now; a->pll_seed = 1; }
                    else a->pll_ema += (off - a->pll_ema) >> g_pll_ema_shift;   /* smooth ±vlag jitter (N6: toward −∞, sub-ms, negligible) */
                    if (ep != a->pll_disturb_seen) {         /* real disturbance → arm a time-boxed one-shot mid-run re-acquire */
                        a->pll_disturb_seen = ep; a->pll_arm_until_us = now + g_pll_startup_us;
                    }
                    /* N7: stability-debounce — fire only when the EMA is LARGE *and* FLAT, so Δ is sized
                     * to the FROZEN bank, not one still forming over the ~1s startup. */
                    ...
                    int in_startup = (now - a->pll_t0_us) < g_pll_startup_us;
                    int armed      = now < a->pll_arm_until_us;
                    int may_acq = a->pll_refractory <= 0 &&
                                  FFABS(a->pll_ema) > (int64_t)g_pll_acquire_us &&
                                  a->pll_dbnc >= g_pll_acquire_n &&
                                  ((in_startup && a->pll_acq_count < g_pll_acquire_k) || armed);  /* N4: startup ≤k, else event-gated */
                    if (may_acq) {
                        int64_t dq = (a->pll_ema / frame_us) * frame_us;   /* quantize to whole frames (toward 0) */
                        if (dq != 0) {
                            ...
                            a->pll_acq_count++;
                            if (g_diag)
                                av_log(NULL, AV_LOG_INFO, "[PTV-PLL] a%d(in%d) ACQUIRE %s %"PRId64"ms (ema→%"PRId64"ms applied=%"PRId64"ms #%d %s)\n",
                                       a->dbg_k, a->dbg_in, dq < 0 ? "drop" : "pad", FFABS(dq) / 1000,
                                       a->pll_ema / 1000, a->af_applied_us / 1000, a->pll_acq_count, in_startup ? "startup" : "event");
                        }
                        a->pll_refractory = g_pll_acquire_n; a->pll_dbnc = 0; a->pll_dbnc_ref = a->pll_ema; a->pll_arm_until_us = 0;
                ...
                if (a->avsync_stat_last == 0) a->avsync_stat_last = nowp;
                    ...
                    if (mv && g_avsync_pll)  /* B3 closed loop: measured offset + integrator state + acquire/guard counts */
                        av_log(NULL, AV_LOG_INFO,
                            ...
                            "  [offset<0 = audio late]\n",
                            a->dbg_k, a->dbg_in, lserr / 1000, m, a->av_vlag_us / 1000, a->av_alag_us / 1000,
                        ...
                        av_log(NULL, AV_LOG_INFO,
                            ...
                            a->dbg_k, a->dbg_in, lserr / 1000, m, a->av_vlag_us / 1000, a->av_alag_us / 1000, lag / 1000);
