...
static int audio_drain_fg(AudioState *a)
{
    ...
    while ((ret = av_buffersink_get_frame(a->afsink, filt)) >= 0) {
        if (filt->pts != AV_NOPTS_VALUE) {
            ...
            if (a->multiview && g_audio_follow && g_af_anchor) {
                ...
                filt->pts = opts + av_rescale(a->af_applied_us, a->out_rate, 1000000);  /* content + smooth follow offset */
