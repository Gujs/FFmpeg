        ...
        if (filt->pts != AV_NOPTS_VALUE) {
            ...
            if (g_stats) {
                ...
                if (a->avsync_stat_last == 0) a->avsync_stat_last = nowp;
                    ...
                    if (mv)              /* multiview: measured offset + the per-slot actuator state */
                        av_log(NULL, AV_LOG_INFO,
                            "[PTV-AVSYNC] a%d(in%d) offset=%s (vlag=%+"PRId64"ms alag=%+"PRId64"ms) "
                            "house_skew=%+"PRId64"ms applied=%+"PRId64"ms err=%+"PRId64"ms nudge=%+"PRId64"ms "
                            "acq[drop=%"PRId64"ms pad=%"PRId64"ms]  [offset<0 = picture ahead of audio]\n",
                            a->dbg_k, a->dbg_in, m, a->av_vlag_us / 1000, a->av_alag_us / 1000,
                            lag / 1000, a->af_applied_us / 1000, (lag - a->af_applied_us) / 1000,
                            a->af_nudge_us / 1000, a->af_acq_drop_us / 1000, a->af_acq_pad_us / 1000);
                    else                 /* single-input: measured offset + the house-clock lock state */
                        av_log(NULL, AV_LOG_INFO,
                            "[PTV-AVSYNC] a%d(in%d) offset=%s (vlag=%+"PRId64"ms alag=%+"PRId64"ms) "
                            "house_skew=%+"PRId64"ms  [offset<0 = picture ahead of audio]\n",
                            a->dbg_k, a->dbg_in, m, a->av_vlag_us / 1000, a->av_alag_us / 1000, lag / 1000);
