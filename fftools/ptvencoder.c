        ...
        if (filt->pts != AV_NOPTS_VALUE) {
            ...
            if (a->multiview && g_audio_follow && g_avsync_pll) {
                ...
                if (a->av_off_valid) {
                    ...
                    if (g_pll_testnoise_us)                  /* TEST-ONLY: ±N square wave (~3.2s flip) to reproduce the box limit cycle locally */
                        off += ((a->out_frames / 150) & 1) ? g_pll_testnoise_us : -g_pll_testnoise_us;
