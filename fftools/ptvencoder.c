...
static void *demux_thread(void *arg)
{
    ...
    while (av_read_frame(d->ifmt, pkt) >= 0) {
        ...
        if (out->stream_index == d->vstream) {
            d->vpkt++;
            if ((out->flags & AV_PKT_FLAG_CORRUPT) && g_discardcorrupt) {   /* +discardcorrupt: count + drop before decode */
                d->vcorrupt++;
                av_packet_free(&out);
                continue;
            }
