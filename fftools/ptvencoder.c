...
static int     g_af_rate_us = 2000;        /* smooth nudge rate ceiling, microseconds per second */
...
static int audio_drain_fg(AudioState *a)
{
    ...
    while ((ret = av_buffersink_get_frame(a->afsink, filt)) >= 0) {
        if (filt->pts != AV_NOPTS_VALUE) {
            ...
            if (a->multiview && g_audio_follow) {
                ...
                if (!a->af_started) { a->af_next_pts = opts; a->af_started = 1; }
                {
                    ...
                    if (!g_af_pll || FFABS(gap) > g_af_acquire_us) {
                        /* ACQUIRE (or PLL disabled): close a large gap with a discrete whole-frame drop/pad
                         * — the startup ramp / big jumps that the ≤rate smooth nudge can't catch in time. */
                        ...
                        /* TRACK (P1): glitch-free smooth correction — a rate-limited sub-sample PTS nudge
                         * (no silence insert, no content drop). Nulls the residual and follows the slow
                         * per-slot drift imperceptibly, so the discrete drop/pad no longer fires every
                         * ~40ms (the "off then OK" hops) and the ±frame residual closes. ≤g_af_rate_us/s. */
            ...
            if (g_stats && a->multiview && g_audio_follow) {
                ...
                if (a->avsync_stat_last == 0) a->avsync_stat_last = nowp;
                    ...
                    av_log(NULL, AV_LOG_INFO,
                        "[PTV-AVSYNC] a%d(in%d) lag=%+"PRId64"ms applied=%+"PRId64"ms err=%+"PRId64"ms "
                        "nudge=%+"PRId64"ms work[acq_drop=%"PRId64"ms acq_pad=%"PRId64"ms]\n",
                        a->dbg_k, a->dbg_in, lag/1000, a->af_applied_us/1000, (lag - a->af_applied_us)/1000,
