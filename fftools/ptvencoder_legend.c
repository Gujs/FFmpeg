...
void ptv_print_log_legend(int full)
{
    ...
    av_log(NULL, AV_LOG_INFO,
        ...
        "  ccheld     (shown when >0) source ERASES refused because the caption had not had its\n"
        "             minimum on-screen time (1.2s). Roll-up sources clear rows constantly, so\n"
        "             obeying every clear showed real captions for only 0.3-0.6s; a held page is\n"
        "             replaced by the next caption or cleared by the encoder 10s stale timeout\n"
        ...
        "             or an encoder error. A steady climb is a real fault.\n"
        "             [PTV-CC] also logs the first caption, QUIET/RESUMED, timeline resets and a\n"
        "             final a53/caps/erase/keep/err/drop/bump/reset tally.\n");
    av_log(NULL, AV_LOG_INFO,
